...
export default async function handler(req) {
  ...
  const { formation, firstName, lastName, email, company, score, maxScore, profile, objectives, answers = [] } = await req.json();

  const notionToken = process.env.NOTION_TOKEN;
  const notionDb    = process.env.NOTION_DB_ID_AUTO;
    ...
    const res = await fetch(slackUrl, {
      ...
      body: JSON.stringify({
        blocks: [
          {
            type: 'header',
            text: { type: 'plain_text', text: '⚙️ Nouveau résultat — Quiz Automatisation IA' },
          },
          {
            type: 'section',
            fields: [
